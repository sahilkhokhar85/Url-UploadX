...
async def download_direct_file(
    *,
    ...
    original_url = parsed_input.source_url

    file_name = (
        parsed_input.custom_file_name
        or _filename_from_url(original_url)
    )
    ...
    timeout = aiohttp.ClientTimeout(
        total=settings.process_max_timeout
    )
    ...
    async with aiohttp.ClientSession(
        ...
        async with session.get(
            ...
            if (
                content_length
                and content_length > 50 * 1024 * 1024
            ):
                raise RuntimeError(
                    f"File too large "
